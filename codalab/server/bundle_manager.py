...
class BundleManager(object):
    ...
    def _schedule_run_bundles_on_workers(self, workers, staged_bundles_to_run, user_info_cache):
        ...
        # We pre-compute the parallel_run_quota left to each user, such that workers that come online
        # or regain the necessary resources while we are attempting to run each staged bundle will
        # respect the ordering of staged_bundles_to_run (i.e., they won't be used immediately, and will
        # be instead assigned bundles on the next run of _run_iteration).
        user_parallel_run_quota_left = {}
        for user in user_queue_positions:
    ...
    def _schedule_run_bundles(self):
        ...
        READY / FAILED, no worker_run DB entry:
            Finished.
        """
        workers = WorkerInfoAccessor(self._worker_model)
